...
- (void)viewDidLoad {
    ...
    if (item.purpose != 0) {
        ...
        self.msg_label.text = @"尊敬的用户，您本次付款金额较高，为保障您的账户安全，需先完成身份认证，再继续支付。";
...
- (void)start_btnAction {
    if (self.agree_btn.selected) {
        ...
        self.nextBlock == nil ? : self.nextBlock();
    }else {
...
//    self.agree_alert_view.url = _protocolLinkUrl;
//
...
//    };
    ...
    SFSafariViewController * vc = [[SFSafariViewController alloc] initWithURL:[NSURL URLWithString:_protocolLinkUrl]];
...
- (void)myAutoLayout {
    ...
    [self.msg_label mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(self.start_btn.mas_top).offset(SCALE_HEIGTHS(-15.0f));
    }];
...
- (UIButton *)face_zc_btn {
    if (!_face_zc_btn) {
        ...
        [_face_zc_btn setTitle:_protocolLinkTxt forState:UIControlStateNormal];
