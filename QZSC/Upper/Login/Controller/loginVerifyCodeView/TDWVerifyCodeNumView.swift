...
class TDWVerifyCodeNumView: UIView {
    ...
    /// 光标颜色
    var cursorColor = UIColor(hexString: "333333")
    
    fileprivate lazy var numLabel: UILabel = {
        let label = UILabel()
        label.textAlignment = .center
        label.font = UIFont.systemFont(ofSize: 40, weight: .medium)
        label.textColor = UIColor(hexString: "1D1D1D")
    ...
    fileprivate lazy var lineView: UIView = {
        let line = UIView()
        line.backgroundColor = UIColor(hexString: "C2C1CF")
    ...
    override init(frame: CGRect) {
        ...
        lineView.snp.makeConstraints { (make) in
            ...
            make.height.equalTo(SCALE_HEIGTHS(value: 3))
        }
...
extension TDWVerifyCodeNumView {
    ...
    func setBottomLineFocus(isFocus: Bool) {
        if isFocus {
            lineView.backgroundColor = UIColor(hexString: "1D1D1D")
            ...
            lineView.backgroundColor = UIColor(hexString: "C2C1CF")
            lineView.snp.remakeConstraints { (make) in
                make.left.bottom.right.equalToSuperview()
