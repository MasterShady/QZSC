...
class TDWVerifyCodeView: UIView {
    ...
    /// 验证码输入框距离两边的边距
    var padding: CGFloat = SCALE_WIDTHS(value: 15)
    
    /// 每个验证码输入框间距
    var spacing: CGFloat = SCALE_WIDTHS(value: 8)
    ...
    func initSubviews() {
        ...
        // 每个验证码框宽度
        let itemWidth: CGFloat = (SCREEN_WIDTH - SCALE_WIDTHS(value: 60) - padding * 2 - spacing * (CGFloat(inputTextNum) - 1)) / CGFloat(inputTextNum)
