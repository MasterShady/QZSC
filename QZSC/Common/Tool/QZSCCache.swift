...
/// 本地缓存管理
public class QZSCCache: NSObject {
    private static let diskConfig = DiskConfig(name: "QZSCCache")
    private static let memoryConfig = MemoryConfig(expiry: .never, countLimit: 10000, totalCostLimit: 10000)
    private static let storage = try? Storage<String, Data>.init(diskConfig: QZSCCache.diskConfig, memoryConfig: QZSCCache.memoryConfig, transformer: TransformerFactory.forData())
