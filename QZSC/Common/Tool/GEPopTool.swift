...
extension UIView{
    ...
    func popDismiss(completedHandler: (()->())? = nil){
...
class GEPopTool: NSObject {
    ...
    static func popViewFormBottom(view:UIView, withMask:Bool = true, tapToDismss: Bool = false){
        ...
        popWindow.backgroundColor = UIColor(hexString: "111111",alpha: withMask ? 0.5 : 0)
    ...
    static func dimssPopView(completedHandler: (()->())? = nil){
    ...
    @objc static func onTapHandle(tap:UITapGestureRecognizer){
    ...
    static var tapGuesture: UITapGestureRecognizer = {
        let tap = UITapGestureRecognizer(target: GEPopTool.self, action: #selector(onTapHandle(tap:)))
