    ...
    func layoutItems() {
        ...
        for  (i,datum) in data.enumerated() {
            ...
            let normalAttrTitle = NSAttributedString(string: title, attributes: [
                ...
                .foregroundColor : UIColor(hexString: "#585960")
            ])
            ...
            let selectedBackImage = UIImage.imageWithColor(color: .black)
            //UIImage(color: .gradient(colors: [.init(hexString: "#00FF98"), .init(hexString: "#00FFED")], from: .zero, to: .init(x: 1, y: 1), size: .init(width: itemW, height: itemH)))
            
            btn.chain.normalAttributedTitle(normalAttrTitle).selectedAttributedTitle(selectedAttrTitle).normalBackgroundImage(.imageWithColor(color: .red)).selectedBackgroundImage(selectedBackImage).corner(radius: 3).clipsToBounds(true)
            ...
            btn.addTarget(self, action: #selector(onSelectClick(sender:)), for: .touchUpInside)
            btn.tag = 1000 + i
...
//            }
    ...
    @objc func onSelectClick(sender: UIButton){
        sender.isSelected = !sender.isSelected
        
        let datum = self.data[sender.tag - 1000]
        if sender.isSelected{
            self.selectedData?.append(datum)
        }else{
            self.selectedData?.remove(datum)
